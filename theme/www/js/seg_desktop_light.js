...
Util.Objects["page"] = new function() {
	...
	// 
		...
		page.buildNode = function(node) {
			// 	
			// 	
		}
		page.initNavigation = function(node) {
			var i, node;
			...
			for(i = 0; node = this.nav_nodes[i]; i++) {
				...
				if (node.submenu) {
					...
					node.onmouseout = function() {
	  					u.as(this.submenu, "display", "none");
	   				}
				}
			}
		}
...
Util.Objects["front"] = new function() {
	this.init = function(scene) {
		scene.nodes = u.qsa(".news li", scene);
		var i, node;
		for(i = 0; node = scene.nodes[i]; i++) {
			...
			u.ie(node, "img", {"src":"/images/"+u.cv(node, "item_id")+"/landscape/320x."+u.cv(node, "i_la")});
		}
	}
}
...
Util.Objects["article"] = new function() {
	this.init = function(scene) {
		var node = u.qs(".item", scene);
		u.ie(node, "img", {"src":"/images/"+u.cv(node, "item_id")+"/landscape/"+ node.offsetWidth +"x."+u.cv(node, "i_la")});
		node.unit = u.qs("dd.unit", node);
		node.unit.innerHTML += "&nbsp;&nbsp;/&nbsp;&nbsp;";
		node.client = u.qs("dd.client", node);
...
Util.Objects["playtype"] = new function() {
	this.init = function(scene) {
		// 
		// 
		this.deco1 = u.ie(scene, "div", {"class":"deco deco1"});
		this.deco1._img = u.ae(this.deco1, "img", {"src":"/img/desktop/playtype_1.jpg"});
		// 	
		// 		
		// 		
		// 	
